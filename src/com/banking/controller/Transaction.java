...
//		response.setHeader("Cache-Control","no-cache , no-store,must-revalidate");
		...
		if (action.equalsIgnoreCase("deposit")) {
			...
			if (type.equalsIgnoreCase("ACCOUNT_ID")) {
				...
				if (accounts.size() > 0) {
					...
					request.setAttribute("message", "Requested Account not exist in Database");
				...
				if (accounts.size() > 1) {
					...
					if (accounts.size() <= 1) {
						...
						request.setAttribute("message", "Requested Customer not exist in Database ");
...

		{
			...
			if (!isMoneyDeposited) {
				request.setAttribute("message", "Money Deposited Successfully");
				request.getRequestDispatcher("/home.jsp").include(request, response);
			} else {
